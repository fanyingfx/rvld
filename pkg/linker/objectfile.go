...
func (o *ObjectFile) IntilizeSections(ctx *Context) {
...
func (o *ObjectFile) FillUpSymtabShndxSec(s *Shdr) {
	...
	o.SymtabShndxSec = utils.ReadSlice[uint32](bs, 32)
}

func (o *ObjectFile) IntializeSymbols(ctx *Context) {
	if o.SymtabSec == nil {
		return
	}
...
func splitSection(ctx *Context, isec *InputSection) *MergeableSection {
	...
	if shdr.Flags&uint64(elf.SHF_STRINGS) != 0 {
		for len(data) > 0 {
			...
			sz := uint64(end + int(shdr.EntSize))
			substr := data[:sz]
			data = data[sz:]
