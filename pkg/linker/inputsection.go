...
func (inputSection *InputSection) ApplyRelocAlloc(ctx *Context, base []byte) {
	rels := inputSection.GetRels()
	for i := 0; i < len(rels); i++ {
		rel := rels[i]
		if rel.Type == uint32(elf.R_RISCV_NONE) || rel.Type == uint32(elf.R_RISCV_RELAX) {
			continue
		}
		sym := inputSection.File.Symbols[rel.Sym]
		loc := base[rel.Offset:]
		if sym.File == nil {
			continue
		}
		S := sym.GetAddr()
		A := uint64(rel.Addend)
		P := inputSection.GetAddr() + rel.Offset
	...
	for a := 0; a < len(rels); a++ {
		...
		case elf.R_RISCV_PCREL_LO12_I, elf.R_RISCV_PCREL_LO12_S:
			sym := inputSection.File.Symbols[rels[a].Sym]
			utils.Assert(sym.InputSection == inputSection)
	...
	for a := 0; a < len(rels); a++ {
		...
		case elf.R_RISCV_PCREL_HI20, elf.R_RISCV_TLS_GOT_HI20:
			...
			utils.Write[uint32](loc, utils.Read[uint32](inputSection.Contents[rels[a].Offset:]))
			writeUtype(loc, val)
		}
	}
}
